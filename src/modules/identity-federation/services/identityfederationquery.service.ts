...
@Injectable()
export class IdentityFederationQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(IdentityFederationQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: IdentityFederationQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${IdentityFederation.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const identityfederations = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de identityfederations obtenido con éxito",
        data: identityfederations,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
          identityfederations.length
        ),
        count: identityfederations.length,
      };
    } catch (error) {
      // Imprimir error
      logger.error(error);
  ...
  async findById(id: string): Promise<IdentityFederationResponse<IdentityFederation>> {
    try {
      const identityfederation = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "IdentityFederations no encontrados para la propiedad y valor especificado"
        );
      // Devolver identityfederation
      return {
        ok: true,
        message: "IdentityFederations obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<IdentityFederationResponse<IdentityFederation>> {
    try {
