...
export class IdentityFederationService {
  constructor(private readonly repository: IdentityFederationRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<IdentityFederation[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<IdentityFederation[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<IdentityFederation[]> {
  ...
  async update(id: string, partialEntity: Partial<IdentityFederation>): Promise<IdentityFederation | null> {
