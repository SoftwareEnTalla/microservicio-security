...
export class IdentityFederationCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: IdentityFederation): Promise<IdentityFederation> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publish(new IdentityFederationCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: IdentityFederation[]): Promise<IdentityFederation[]> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publishAll(result.map((el)=>new IdentityFederationCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      this.eventPublisher.publish(new IdentityFederationUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            this.eventPublisher.publish(new IdentityFederationUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       this.eventPublisher.publish(new IdentityFederationDeletedEvent(id, {
        instance: entity,
        metadata: {
