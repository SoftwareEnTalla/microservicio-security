...
@ApiTags("ChallengeStatus Query")
@UseGuards(ChallengeStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("challengestatuss/query")
export class ChallengeStatusQueryController {
  #logger = new Logger(ChallengeStatusQueryController.name);

  constructor(private readonly service: ChallengeStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all challengestatus with optional pagination" })
  @ApiResponse({ status: 200, type: ChallengeStatussResponse })
  @ApiQuery({ name: "options", required: false, type: ChallengeStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ChallengeStatus>    
  ): Promise<ChallengeStatussResponse<ChallengeStatus>> {
    try {
     
      const challengestatuss = await this.service.findAll(options);
      logger.info("Retrieving all challengestatus");
  ...
  @ApiOperation({ summary: "Find challengestatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter challengestatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ChallengeStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ChallengeStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find challengestatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ChallengeStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all challengestatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count challengestatuss with conditions" })
  @ApiResponse({ status: 200, type: ChallengeStatussResponse<ChallengeStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one challengestatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one challengestatus or return error" })
  @ApiResponse({ status: 200, type: ChallengeStatusResponse<ChallengeStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get challengestatus by ID" })
  @ApiResponse({ status: 200, type: ChallengeStatusResponse<ChallengeStatus> })
  @ApiResponse({ status: 404, description: "ChallengeStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the challengestatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ChallengeStatusResponse<ChallengeStatus>> {
    try {
      const challengestatus = await this.service.findOne({ where: { id } });
      if (!challengestatus) {
        throw new NotFoundException(
