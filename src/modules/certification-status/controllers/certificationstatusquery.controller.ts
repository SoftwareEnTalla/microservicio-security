...
import { CertificationStatusDto } from "../dtos/all-dto";
...
@ApiTags("CertificationStatus Query")
@UseGuards(CertificationStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("certificationstatuss/query")
export class CertificationStatusQueryController {
  #logger = new Logger(CertificationStatusQueryController.name);

  constructor(private readonly service: CertificationStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all certificationstatus with optional pagination" })
  @ApiResponse({ status: 200, type: CertificationStatussResponse })
  @ApiQuery({ name: "options", required: false, type: CertificationStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<CertificationStatus>    
  ): Promise<CertificationStatussResponse<CertificationStatus>> {
    try {
     
      const certificationstatuss = await this.service.findAll(options);
      logger.info("Retrieving all certificationstatus");
  ...
  @ApiOperation({ summary: "Find certificationstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter certificationstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: CertificationStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "CertificationStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find certificationstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<CertificationStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all certificationstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count certificationstatuss with conditions" })
  @ApiResponse({ status: 200, type: CertificationStatussResponse<CertificationStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades CertificationStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one certificationstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one certificationstatus or return error" })
  @ApiResponse({ status: 200, type: CertificationStatusResponse<CertificationStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad CertificationStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get certificationstatus by ID" })
  @ApiResponse({ status: 200, type: CertificationStatusResponse<CertificationStatus> })
  @ApiResponse({ status: 404, description: "CertificationStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the certificationstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<CertificationStatusResponse<CertificationStatus>> {
    try {
      const certificationstatus = await this.service.findOne({ where: { id } });
      if (!certificationstatus) {
        throw new NotFoundException(
          "CertificationStatus no encontrado para el id solicitado"
        );
      }
