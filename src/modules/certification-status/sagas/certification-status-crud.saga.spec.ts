...
import { CertificationStatusCrudSaga } from './certification-status-crud.saga';
import { CertificationStatusCreatedEvent } from '../events/certificationstatuscreated.event';

describe('CertificationStatusCrudSaga', () => {
  it('reacciona al evento de creación sin romper el flujo CQRS', async () => {
    const saga = new CertificationStatusCrudSaga({ execute: jest.fn() } as any, { publish: jest.fn() } as any);
    ...
    const result = await lastValueFrom(saga.onCertificationStatusCreated(of(event)));
