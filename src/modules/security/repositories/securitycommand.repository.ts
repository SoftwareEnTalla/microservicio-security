...
export class SecurityCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: Security): Promise<Security> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvt1 = new SecurityCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: Security[]): Promise<Security[]> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvts2 = result.map((el)=>new SecurityCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      const __dualEvt3 = new SecurityUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            const __dualEvt4 = new SecurityUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       const __dualEvt5 = new SecurityDeletedEvent(id, {
        instance: entity,
        metadata: {
