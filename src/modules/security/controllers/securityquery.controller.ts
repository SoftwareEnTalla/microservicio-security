...
@ApiTags("Security Query")
@UseGuards(SecurityAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("securitys/query")
export class SecurityQueryController {
  #logger = new Logger(SecurityQueryController.name);

  constructor(private readonly service: SecurityQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all security with optional pagination" })
  @ApiResponse({ status: 200, type: SecuritysResponse })
  @ApiQuery({ name: "options", required: false, type: SecurityDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Security>    
  ): Promise<SecuritysResponse<Security>> {
    try {
     
      const securitys = await this.service.findAll(options);
      logger.info("Retrieving all security");
      return securitys;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the security to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<SecurityResponse<Security>> {
    try {
      const security = await this.service.findOne({ where: { id } });
      if (!security) {
        throw new NotFoundException(
          "Security no encontrado para el id solicitado"
        );
      }
      return security;
    } catch (error) {
  ...
  @ApiOperation({ summary: "Find security by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter security', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: SecuritysResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Security no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find securitys with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Security>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all securitys" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count securitys with conditions" })
  @ApiResponse({ status: 200, type: SecuritysResponse<Security> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Securitys no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one security with conditions" })
  @ApiResponse({ status: 200, type: SecurityResponse<Security> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiOperation({ summary: "Find one security or return error" })
  @ApiResponse({ status: 200, type: SecurityResponse<Security> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Security no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
