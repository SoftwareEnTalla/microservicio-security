...
@ApiTags("AuthMethod Query")
@UseGuards(AuthMethodAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("authmethods/query")
export class AuthMethodQueryController {
  #logger = new Logger(AuthMethodQueryController.name);

  constructor(private readonly service: AuthMethodQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all authmethod with optional pagination" })
  @ApiResponse({ status: 200, type: AuthMethodsResponse })
  @ApiQuery({ name: "options", required: false, type: AuthMethodDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<AuthMethod>    
  ): Promise<AuthMethodsResponse<AuthMethod>> {
    try {
     
      const authmethods = await this.service.findAll(options);
      logger.info("Retrieving all authmethod");
      return authmethods;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find authmethod by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter authmethod', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: AuthMethodsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "AuthMethod no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find authmethods with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<AuthMethod>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all authmethods" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count authmethods with conditions" })
  @ApiResponse({ status: 200, type: AuthMethodsResponse<AuthMethod> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades AuthMethods no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one authmethod with conditions" })
  ...
  @ApiOperation({ summary: "Find one authmethod or return error" })
  @ApiResponse({ status: 200, type: AuthMethodResponse<AuthMethod> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad AuthMethod no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get authmethod by ID" })
  @ApiResponse({ status: 200, type: AuthMethodResponse<AuthMethod> })
  @ApiResponse({ status: 404, description: "AuthMethod not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the authmethod to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<AuthMethodResponse<AuthMethod>> {
    try {
      const authmethod = await this.service.findOne({ where: { id } });
      if (!authmethod) {
        throw new NotFoundException(
          "AuthMethod no encontrado para el id solicitado"
        );
      }
      return authmethod;
    } catch (error) {
