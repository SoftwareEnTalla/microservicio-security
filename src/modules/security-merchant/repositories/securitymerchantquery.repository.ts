...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class SecurityMerchantQueryRepository {

    //Constructor del repositorio de datos: SecurityMerchantQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination SecurityMerchant on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[SecurityMerchant[], number]> {
      logger.info('Ready to findAndCount SecurityMerchant on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of SecurityMerchant on repository:',result);
      return result;
    }
    ...
    client: LoggerClient.getInstance()
      ...
      .get(SecurityMerchantRepository.name),
        ...
        async findOne(options?: Record<string, any>): Promise<SecurityMerchant | null> {
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) SecurityMerchant:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<SecurityMerchant> = options as FindOptionsWhere<SecurityMerchant>;
