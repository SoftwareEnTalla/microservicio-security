...
@Injectable()
export class SecurityMerchantService {
  constructor(private readonly repository: SecurityMerchantRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<SecurityMerchant[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<SecurityMerchant[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<SecurityMerchant[]> {
  ...
  async update(id: string, partialEntity: Partial<SecurityMerchant>): Promise<SecurityMerchant | null> {
