...
@Injectable()
export class SystemAdminPolicyService {
  constructor(private readonly repository: SystemAdminPolicyRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<SystemAdminPolicy[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<SystemAdminPolicy[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<SystemAdminPolicy[]> {
  ...
  async update(id: string, partialEntity: Partial<SystemAdminPolicy>): Promise<SystemAdminPolicy | null> {
