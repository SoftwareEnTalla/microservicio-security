...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class SystemAdminPolicyQueryRepository {

    //Constructor del repositorio de datos: SystemAdminPolicyQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination SystemAdminPolicy on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[SystemAdminPolicy[], number]> {
      logger.info('Ready to findAndCount SystemAdminPolicy on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of SystemAdminPolicy on repository:',result);
      return result;
    }
    ...
    async findOne(where?: Record<string, any>): Promise<SystemAdminPolicy | null> {
      const tmp: FindOptionsWhere<SystemAdminPolicy> = where as FindOptionsWhere<SystemAdminPolicy>;
      logger.info('Ready to findOneBy SystemAdminPolicy on repository with conditions:', tmp);
