...
@ApiTags("UserType Query")
@UseGuards(UserTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("usertypes/query")
export class UserTypeQueryController {
  #logger = new Logger(UserTypeQueryController.name);

  constructor(private readonly service: UserTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all usertype with optional pagination" })
  @ApiResponse({ status: 200, type: UserTypesResponse })
  @ApiQuery({ name: "options", required: false, type: UserTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<UserType>    
  ): Promise<UserTypesResponse<UserType>> {
    try {
     
      const usertypes = await this.service.findAll(options);
      logger.info("Retrieving all usertype");
      return usertypes;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find usertype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter usertype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: UserTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "UserType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find usertypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<UserType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all usertypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count usertypes with conditions" })
  @ApiResponse({ status: 200, type: UserTypesResponse<UserType> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one usertype with conditions" })
  ...
  @ApiOperation({ summary: "Find one usertype or return error" })
  @ApiResponse({ status: 200, type: UserTypeResponse<UserType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get usertype by ID" })
  @ApiResponse({ status: 200, type: UserTypeResponse<UserType> })
  @ApiResponse({ status: 404, description: "UserType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the usertype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<UserTypeResponse<UserType>> {
    try {
      const usertype = await this.service.findOne({ where: { id } });
      if (!usertype) {
        throw new NotFoundException(
