...
import { logger } from '@core/logs/logger';
  ...
  export class LoginQueryRepository {
    ...
    //Constructor del repositorio de datos: LoginQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination Login on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[Login[], number]> {
      logger.info('Ready to findAndCount Login on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of Login on repository:',result);
      return result;
    }
