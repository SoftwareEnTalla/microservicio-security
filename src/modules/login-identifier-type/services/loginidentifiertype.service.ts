...
@Injectable()
export class LoginIdentifierTypeService {
  constructor(private readonly repository: LoginIdentifierTypeRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<LoginIdentifierType[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<LoginIdentifierType[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<LoginIdentifierType[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<LoginIdentifierType | null> {
  ...
  async update(id: string, partialEntity: Partial<LoginIdentifierType>): Promise<LoginIdentifierType | null> {
