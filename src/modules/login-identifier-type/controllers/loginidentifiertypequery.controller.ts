...
@ApiTags("LoginIdentifierType Query")
@UseGuards(LoginIdentifierTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("loginidentifiertypes/query")
export class LoginIdentifierTypeQueryController {
  #logger = new Logger(LoginIdentifierTypeQueryController.name);

  constructor(private readonly service: LoginIdentifierTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all loginidentifiertype with optional pagination" })
  @ApiResponse({ status: 200, type: LoginIdentifierTypesResponse })
  @ApiQuery({ name: "options", required: false, type: LoginIdentifierTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<LoginIdentifierType>    
  ): Promise<LoginIdentifierTypesResponse<LoginIdentifierType>> {
    try {
     
      const loginidentifiertypes = await this.service.findAll(options);
      logger.info("Retrieving all loginidentifiertype");
  ...
  @ApiOperation({ summary: "Find loginidentifiertype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter loginidentifiertype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: LoginIdentifierTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "LoginIdentifierType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find loginidentifiertypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<LoginIdentifierType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all loginidentifiertypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count loginidentifiertypes with conditions" })
  @ApiResponse({ status: 200, type: LoginIdentifierTypesResponse<LoginIdentifierType> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one loginidentifiertype with conditions" })
  ...
  @ApiOperation({ summary: "Find one loginidentifiertype or return error" })
  @ApiResponse({ status: 200, type: LoginIdentifierTypeResponse<LoginIdentifierType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get loginidentifiertype by ID" })
  @ApiResponse({ status: 200, type: LoginIdentifierTypeResponse<LoginIdentifierType> })
  @ApiResponse({ status: 404, description: "LoginIdentifierType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the loginidentifiertype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<LoginIdentifierTypeResponse<LoginIdentifierType>> {
    try {
      const loginidentifiertype = await this.service.findOne({ where: { id } });
