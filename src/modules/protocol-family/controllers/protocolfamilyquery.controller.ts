...
import { ProtocolFamilyDto } from "../dtos/all-dto";
...
@ApiTags("ProtocolFamily Query")
@UseGuards(ProtocolFamilyAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("protocolfamilys/query")
export class ProtocolFamilyQueryController {
  #logger = new Logger(ProtocolFamilyQueryController.name);

  constructor(private readonly service: ProtocolFamilyQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all protocolfamily with optional pagination" })
  @ApiResponse({ status: 200, type: ProtocolFamilysResponse })
  @ApiQuery({ name: "options", required: false, type: ProtocolFamilyDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ProtocolFamily>    
  ): Promise<ProtocolFamilysResponse<ProtocolFamily>> {
    try {
     
      const protocolfamilys = await this.service.findAll(options);
      logger.info("Retrieving all protocolfamily");
  ...
  @ApiOperation({ summary: "Find protocolfamily by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter protocolfamily', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ProtocolFamilysResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ProtocolFamily no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find protocolfamilys with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ProtocolFamily>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all protocolfamilys" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count protocolfamilys with conditions" })
  @ApiResponse({ status: 200, type: ProtocolFamilysResponse<ProtocolFamily> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ProtocolFamilys no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one protocolfamily with conditions" })
  ...
  @ApiOperation({ summary: "Find one protocolfamily or return error" })
  @ApiResponse({ status: 200, type: ProtocolFamilyResponse<ProtocolFamily> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ProtocolFamily no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get protocolfamily by ID" })
  @ApiResponse({ status: 200, type: ProtocolFamilyResponse<ProtocolFamily> })
  @ApiResponse({ status: 404, description: "ProtocolFamily not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the protocolfamily to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ProtocolFamilyResponse<ProtocolFamily>> {
    try {
      const protocolfamily = await this.service.findOne({ where: { id } });
      if (!protocolfamily) {
        throw new NotFoundException(
          "ProtocolFamily no encontrado para el id solicitado"
        );
      }
