...
@Injectable()
export class UserProfileQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(UserProfileQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: UserProfileQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${UserProfile.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const userprofiles = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de userprofiles obtenido con éxito",
        data: userprofiles,
  ...
  async findById(id: string): Promise<UserProfileResponse<UserProfile>> {
    try {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "UserProfiles no encontrados para la propiedad y valor especificado"
        );
      // Devolver userprofile
      return {
        ok: true,
        message: "UserProfiles obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<UserProfileResponse<UserProfile>> {
    try {
