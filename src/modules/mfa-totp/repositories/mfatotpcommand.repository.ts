...
export class MfaTotpCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: MfaTotp): Promise<MfaTotp> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publish(new MfaTotpCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: MfaTotp[]): Promise<MfaTotp[]> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publishAll(result.map((el)=>new MfaTotpCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      this.eventPublisher.publish(new MfaTotpUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            this.eventPublisher.publish(new MfaTotpUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       this.eventPublisher.publish(new MfaTotpDeletedEvent(id, {
        instance: entity,
        metadata: {
