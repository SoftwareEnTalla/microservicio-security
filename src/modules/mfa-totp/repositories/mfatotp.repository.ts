...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class MfaTotpRepository {
    constructor(
    ...
    async findAndCount(where?: Record<string, any>): Promise<[MfaTotp[], number]> {
      logger.info('Ready to findByField MfaTotp on repository:',where);
    ...
    client: LoggerClient.getInstance()
      ...
      .get(MfaTotpRepository.name),
        ...
        async findOne(options?: Record<string, any>): Promise<MfaTotp | null> {
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) MfaTotp:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<MfaTotp> = options as FindOptionsWhere<MfaTotp>;
    ...
    async update(
        ...
        logger.info('Updated instance of MfaTotp with id:  was finded on repository:', instance);
