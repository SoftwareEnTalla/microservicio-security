...
@ApiTags("AccountStatus Query")
@UseGuards(AccountStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("accountstatuss/query")
export class AccountStatusQueryController {
  #logger = new Logger(AccountStatusQueryController.name);

  constructor(private readonly service: AccountStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all accountstatus with optional pagination" })
  @ApiResponse({ status: 200, type: AccountStatussResponse })
  @ApiQuery({ name: "options", required: false, type: AccountStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<AccountStatus>    
  ): Promise<AccountStatussResponse<AccountStatus>> {
    try {
     
      const accountstatuss = await this.service.findAll(options);
      logger.info("Retrieving all accountstatus");
  ...
  @ApiOperation({ summary: "Find accountstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter accountstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: AccountStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "AccountStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find accountstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<AccountStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all accountstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count accountstatuss with conditions" })
  @ApiResponse({ status: 200, type: AccountStatussResponse<AccountStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one accountstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one accountstatus or return error" })
  @ApiResponse({ status: 200, type: AccountStatusResponse<AccountStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get accountstatus by ID" })
  @ApiResponse({ status: 200, type: AccountStatusResponse<AccountStatus> })
  @ApiResponse({ status: 404, description: "AccountStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the accountstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<AccountStatusResponse<AccountStatus>> {
    try {
      const accountstatus = await this.service.findOne({ where: { id } });
      if (!accountstatus) {
        throw new NotFoundException(
