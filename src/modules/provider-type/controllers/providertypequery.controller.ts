...
import { ProviderTypeDto } from "../dtos/all-dto";
...
@ApiTags("ProviderType Query")
@UseGuards(ProviderTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("providertypes/query")
export class ProviderTypeQueryController {
  #logger = new Logger(ProviderTypeQueryController.name);

  constructor(private readonly service: ProviderTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all providertype with optional pagination" })
  @ApiResponse({ status: 200, type: ProviderTypesResponse })
  @ApiQuery({ name: "options", required: false, type: ProviderTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ProviderType>    
  ): Promise<ProviderTypesResponse<ProviderType>> {
    try {
     
      const providertypes = await this.service.findAll(options);
      logger.info("Retrieving all providertype");
  ...
  @ApiOperation({ summary: "Find providertype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter providertype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ProviderTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ProviderType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find providertypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ProviderType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all providertypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count providertypes with conditions" })
  @ApiResponse({ status: 200, type: ProviderTypesResponse<ProviderType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ProviderTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one providertype with conditions" })
  ...
  @ApiOperation({ summary: "Find one providertype or return error" })
  @ApiResponse({ status: 200, type: ProviderTypeResponse<ProviderType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ProviderType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get providertype by ID" })
  @ApiResponse({ status: 200, type: ProviderTypeResponse<ProviderType> })
  @ApiResponse({ status: 404, description: "ProviderType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the providertype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ProviderTypeResponse<ProviderType>> {
    try {
      const providertype = await this.service.findOne({ where: { id } });
      if (!providertype) {
        throw new NotFoundException(
          "ProviderType no encontrado para el id solicitado"
        );
      }
      return providertype;
    } catch (error) {
