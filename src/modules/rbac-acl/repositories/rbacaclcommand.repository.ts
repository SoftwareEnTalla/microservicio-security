...
export class RbacAclCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: RbacAcl): Promise<RbacAcl> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvt1 = new RbacAclCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: RbacAcl[]): Promise<RbacAcl[]> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvts2 = result.map((el)=>new RbacAclCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      const __dualEvt3 = new RbacAclUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            const __dualEvt4 = new RbacAclUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       const __dualEvt5 = new RbacAclDeletedEvent(id, {
        instance: entity,
        metadata: {
