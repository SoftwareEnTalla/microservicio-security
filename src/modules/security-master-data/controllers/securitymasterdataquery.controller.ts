...
import { SecurityMasterDataDto } from "../dtos/all-dto";
...
export class SecurityMasterDataQueryController {
  ...
  constructor(private readonly service: SecurityMasterDataQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all securitymasterdata with optional pagination" })
  @ApiResponse({ status: 200, type: SecurityMasterDatasResponse })
  @ApiQuery({ name: "options", required: false, type: SecurityMasterDataDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<SecurityMasterData>    
  ): Promise<SecurityMasterDatasResponse<SecurityMasterData>> {
    try {
     
      const securitymasterdatas = await this.service.findAll(options);
      logger.info("Retrieving all securitymasterdata");
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get securitymasterdata by ID" })
  @ApiResponse({ status: 200, type: SecurityMasterDataResponse<SecurityMasterData> })
  @ApiResponse({ status: 404, description: "SecurityMasterData not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the securitymasterdata to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<SecurityMasterDataResponse<SecurityMasterData>> {
    try {
      const securitymasterdata = await this.service.findOne({ where: { id } });
  ...
  @ApiOperation({ summary: "Find securitymasterdata by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter securitymasterdata', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: SecurityMasterDatasResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "SecurityMasterData no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find securitymasterdatas with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<SecurityMasterData>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all securitymasterdatas" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count securitymasterdatas with conditions" })
  @ApiResponse({ status: 200, type: SecurityMasterDatasResponse<SecurityMasterData> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one securitymasterdata with conditions" })
  @ApiResponse({ status: 200, type: SecurityMasterDataResponse<SecurityMasterData> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiOperation({ summary: "Find one securitymasterdata or return error" })
  @ApiResponse({ status: 200, type: SecurityMasterDataResponse<SecurityMasterData> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
