...
import {
  SessionTokenCreatedEvent,
  SessionTokenUpdatedEvent,
  SessionTokenDeletedEvent,

} from '../events/exporting.event';
import {
  SagaSessionTokenFailedEvent
} from '../events/sessiontoken-failed.event';
import {
...
export class SessionTokenCrudSaga {
  ...
  @Saga()
  onSessionTokenCreated = ($events: Observable<SessionTokenCreatedEvent>) => {
    return $events.pipe(
  ...
  @Saga()
  onSessionTokenUpdated = ($events: Observable<SessionTokenUpdatedEvent>) => {
    return $events.pipe(
  ...
  @Saga()
  onSessionTokenDeleted = ($events: Observable<SessionTokenDeletedEvent>) => {
    return $events.pipe(
  ...
  private handleSagaError(error: Error, event: any) {
    ...
    this.eventBus.publish(new SagaSessionTokenFailedEvent( error,event));
  }
}
