...
import { FlowTypeDto } from "../dtos/all-dto";
...
@ApiTags("FlowType Query")
@UseGuards(FlowTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("flowtypes/query")
export class FlowTypeQueryController {
  #logger = new Logger(FlowTypeQueryController.name);

  constructor(private readonly service: FlowTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all flowtype with optional pagination" })
  @ApiResponse({ status: 200, type: FlowTypesResponse })
  @ApiQuery({ name: "options", required: false, type: FlowTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<FlowType>    
  ): Promise<FlowTypesResponse<FlowType>> {
    try {
     
      const flowtypes = await this.service.findAll(options);
      logger.info("Retrieving all flowtype");
      return flowtypes;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find flowtype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter flowtype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: FlowTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "FlowType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find flowtypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<FlowType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all flowtypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count flowtypes with conditions" })
  @ApiResponse({ status: 200, type: FlowTypesResponse<FlowType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades FlowTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one flowtype with conditions" })
  ...
  @ApiOperation({ summary: "Find one flowtype or return error" })
  @ApiResponse({ status: 200, type: FlowTypeResponse<FlowType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad FlowType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get flowtype by ID" })
  @ApiResponse({ status: 200, type: FlowTypeResponse<FlowType> })
  @ApiResponse({ status: 404, description: "FlowType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the flowtype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<FlowTypeResponse<FlowType>> {
    try {
      const flowtype = await this.service.findOne({ where: { id } });
      if (!flowtype) {
        throw new NotFoundException(
          "FlowType no encontrado para el id solicitado"
        );
      }
      return flowtype;
    } catch (error) {
