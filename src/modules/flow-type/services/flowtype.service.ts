...
export class FlowTypeService {
  constructor(private readonly repository: FlowTypeRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<FlowType[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<FlowType[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<FlowType[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<FlowType | null> {
  ...
  async update(id: string, partialEntity: Partial<FlowType>): Promise<FlowType | null> {
