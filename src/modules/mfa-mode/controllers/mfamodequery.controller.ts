...
@ApiTags("MfaMode Query")
@UseGuards(MfaModeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("mfamodes/query")
export class MfaModeQueryController {
  #logger = new Logger(MfaModeQueryController.name);

  constructor(private readonly service: MfaModeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all mfamode with optional pagination" })
  @ApiResponse({ status: 200, type: MfaModesResponse })
  @ApiQuery({ name: "options", required: false, type: MfaModeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<MfaMode>    
  ): Promise<MfaModesResponse<MfaMode>> {
    try {
     
      const mfamodes = await this.service.findAll(options);
      logger.info("Retrieving all mfamode");
      return mfamodes;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find mfamode by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter mfamode', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: MfaModesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "MfaMode no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find mfamodes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<MfaMode>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all mfamodes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count mfamodes with conditions" })
  @ApiResponse({ status: 200, type: MfaModesResponse<MfaMode> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one mfamode with conditions" })
  ...
  @ApiOperation({ summary: "Find one mfamode or return error" })
  @ApiResponse({ status: 200, type: MfaModeResponse<MfaMode> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get mfamode by ID" })
  @ApiResponse({ status: 200, type: MfaModeResponse<MfaMode> })
  @ApiResponse({ status: 404, description: "MfaMode not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the mfamode to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<MfaModeResponse<MfaMode>> {
    try {
      const mfamode = await this.service.findOne({ where: { id } });
      if (!mfamode) {
        throw new NotFoundException(
