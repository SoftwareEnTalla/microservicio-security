...
export class SecurityCustomerCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: SecurityCustomer): Promise<SecurityCustomer> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publish(new SecurityCustomerCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: SecurityCustomer[]): Promise<SecurityCustomer[]> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publishAll(result.map((el)=>new SecurityCustomerCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      this.eventPublisher.publish(new SecurityCustomerUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            this.eventPublisher.publish(new SecurityCustomerUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       this.eventPublisher.publish(new SecurityCustomerDeletedEvent(id, {
        instance: entity,
        metadata: {
