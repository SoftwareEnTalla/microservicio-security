...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class IdentifierTypeRepository {
    constructor(
    ...
    async findAll(options?: FindManyOptions<IdentifierType>): Promise<IdentifierType[]> {
      logger.info('Ready to findAll IdentifierType on repository:',options);
    ...
    async findById(id: string): Promise<IdentifierType | null> {
      const tmp: FindOptionsWhere<IdentifierType> = { id } as FindOptionsWhere<IdentifierType>;
      logger.info('Ready to findById IdentifierType on repository:',tmp);
    ...
    async findByField(
      ...
      logger.info('Ready to findByField IdentifierType on repository:',options);
    ...
    async count(): Promise<number> {
      logger.info('Ready to count IdentifierType on repository...');
      let result= this.repository.count();
      logger.info('Was counted  instances of IdentifierType on repository:');
      return result;
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[IdentifierType[], number]> {
      logger.info('Ready to findByField IdentifierType on repository:',where);
    ...
    async findOne(options?: Record<string, any>): Promise<IdentifierType | null> {
      if (!options || Object.keys(options).length === 0) {
        logger.warn('No conditions provided for finding IdentifierType.');
        return null;
      }
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) IdentifierType:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<IdentifierType> = options as FindOptionsWhere<IdentifierType>;
      logger.info('Ready to findOneBy IdentifierType on repository:', tmp);
      return this.repository.findOneBy(tmp);
    }
    ...
    async findOneOrFail(where?: Record<string, any>): Promise<IdentifierType> {
      logger.info('Ready to findOneOrFail IdentifierType on repository:',where);
    ...
    async update(
        ...
        logger.info('Updated instance of IdentifierType with id:  was finded on repository:', instance);
