...
@ApiTags("TokenType Query")
@UseGuards(TokenTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("tokentypes/query")
export class TokenTypeQueryController {
  #logger = new Logger(TokenTypeQueryController.name);

  constructor(private readonly service: TokenTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all tokentype with optional pagination" })
  @ApiResponse({ status: 200, type: TokenTypesResponse })
  @ApiQuery({ name: "options", required: false, type: TokenTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<TokenType>    
  ): Promise<TokenTypesResponse<TokenType>> {
    try {
     
      const tokentypes = await this.service.findAll(options);
      logger.info("Retrieving all tokentype");
      return tokentypes;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find tokentype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter tokentype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: TokenTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "TokenType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find tokentypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<TokenType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all tokentypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count tokentypes with conditions" })
  @ApiResponse({ status: 200, type: TokenTypesResponse<TokenType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades TokenTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one tokentype with conditions" })
  ...
  @ApiOperation({ summary: "Find one tokentype or return error" })
  @ApiResponse({ status: 200, type: TokenTypeResponse<TokenType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad TokenType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get tokentype by ID" })
  @ApiResponse({ status: 200, type: TokenTypeResponse<TokenType> })
  @ApiResponse({ status: 404, description: "TokenType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the tokentype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<TokenTypeResponse<TokenType>> {
    try {
      const tokentype = await this.service.findOne({ where: { id } });
      if (!tokentype) {
        throw new NotFoundException(
          "TokenType no encontrado para el id solicitado"
        );
      }
      return tokentype;
    } catch (error) {
