...
import { AuthStatusDto } from "../dtos/all-dto";
...
@ApiTags("AuthStatus Query")
@UseGuards(AuthStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("authstatuss/query")
export class AuthStatusQueryController {
  #logger = new Logger(AuthStatusQueryController.name);

  constructor(private readonly service: AuthStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all authstatus with optional pagination" })
  @ApiResponse({ status: 200, type: AuthStatussResponse })
  @ApiQuery({ name: "options", required: false, type: AuthStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<AuthStatus>    
  ): Promise<AuthStatussResponse<AuthStatus>> {
    try {
     
      const authstatuss = await this.service.findAll(options);
      logger.info("Retrieving all authstatus");
      return authstatuss;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find authstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter authstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: AuthStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "AuthStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find authstatuss with pagination" })
  @ApiResponse({ status: 200, type: AuthStatussResponse<AuthStatus> })
  @ApiQuery({ name: "options", required: false, type: AuthStatusDto }) // Ajustar según el tipo real
  @ApiQuery({ name: "page", required: false, type: Number })
  @ApiQuery({ name: "size", required: false, type: Number })
  @ApiQuery({ name: "sort", required: false, type: String })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<AuthStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all authstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count authstatuss with conditions" })
  @ApiResponse({ status: 200, type: AuthStatussResponse<AuthStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades AuthStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one authstatus with conditions" })
  @ApiResponse({ status: 200, type: AuthStatusResponse<AuthStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  @ApiOperation({ summary: "Find one authstatus or return error" })
  @ApiResponse({ status: 200, type: AuthStatusResponse<AuthStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad AuthStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get authstatus by ID" })
  @ApiResponse({ status: 200, type: AuthStatusResponse<AuthStatus> })
  @ApiResponse({ status: 404, description: "AuthStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the authstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<AuthStatusResponse<AuthStatus>> {
    try {
      const authstatus = await this.service.findOne({ where: { id } });
      if (!authstatus) {
        throw new NotFoundException(
          "AuthStatus no encontrado para el id solicitado"
        );
      }
      return authstatus;
    } catch (error) {
