...
import { ChallengeTypeDto } from "../dtos/all-dto";
...
@ApiTags("ChallengeType Query")
@UseGuards(ChallengeTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("challengetypes/query")
export class ChallengeTypeQueryController {
  #logger = new Logger(ChallengeTypeQueryController.name);

  constructor(private readonly service: ChallengeTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all challengetype with optional pagination" })
  @ApiResponse({ status: 200, type: ChallengeTypesResponse })
  @ApiQuery({ name: "options", required: false, type: ChallengeTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ChallengeType>    
  ): Promise<ChallengeTypesResponse<ChallengeType>> {
    try {
     
      const challengetypes = await this.service.findAll(options);
      logger.info("Retrieving all challengetype");
  ...
  @ApiOperation({ summary: "Find challengetype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter challengetype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ChallengeTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ChallengeType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find challengetypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ChallengeType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all challengetypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count challengetypes with conditions" })
  @ApiResponse({ status: 200, type: ChallengeTypesResponse<ChallengeType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ChallengeTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one challengetype with conditions" })
  ...
  @ApiOperation({ summary: "Find one challengetype or return error" })
  @ApiResponse({ status: 200, type: ChallengeTypeResponse<ChallengeType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ChallengeType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get challengetype by ID" })
  @ApiResponse({ status: 200, type: ChallengeTypeResponse<ChallengeType> })
  @ApiResponse({ status: 404, description: "ChallengeType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the challengetype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ChallengeTypeResponse<ChallengeType>> {
    try {
      const challengetype = await this.service.findOne({ where: { id } });
      if (!challengetype) {
        throw new NotFoundException(
          "ChallengeType no encontrado para el id solicitado"
        );
      }
