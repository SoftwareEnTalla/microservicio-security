...
export class UserQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(UserQueryService.name);
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${User.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      ...
      return {
        ok: true,
        message: "Listado de users obtenido con éxito",
        data: users,
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "Users no encontrados para la propiedad y valor especificado"
        );
      // Devolver user
      return {
        ok: true,
        message: "Users obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
