...
export class AuthenticationCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: Authentication): Promise<Authentication> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvt1 = new AuthenticationCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: Authentication[]): Promise<Authentication[]> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvts2 = result.map((el)=>new AuthenticationCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      const __dualEvt3 = new AuthenticationUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            const __dualEvt4 = new AuthenticationUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       const __dualEvt5 = new AuthenticationDeletedEvent(id, {
        instance: entity,
        metadata: {
