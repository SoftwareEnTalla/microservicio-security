 ...
 * Licencia: MIT
 *
 * OnAuthMethod2ChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `auth-method-2` (owner: security). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'auth-method-2';
const OWNER_BC = 'security';
const CROSS_CONTEXT = true;
const EVENT_NAMES: string[] = [
  'AuthMethod2CreatedEvent',
  'AuthMethod2UpdatedEvent',
  'AuthMethod2DeletedEvent',
];
const TOPICS: string[] = [
  'auth-method-2-created',
  'auth-method-2-updated',
  'auth-method-2-deleted',
];

@Injectable()
export class OnAuthMethod2ChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnAuthMethod2ChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnAuthMethod2ChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnAuthMethod2ChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onAuthMethod2Change] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
