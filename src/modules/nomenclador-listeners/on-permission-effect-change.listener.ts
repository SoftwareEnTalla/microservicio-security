 ...
 * Licencia: MIT
 *
 * OnPermissionEffectChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `permission-effect` (owner: catalog). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'permission-effect';
...
const TOPICS: string[] = [
  'permission-effect-created',
  'permission-effect-updated',
  'permission-effect-deleted',
];

@Injectable()
export class OnPermissionEffectChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnPermissionEffectChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnPermissionEffectChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnPermissionEffectChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onPermissionEffectChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
