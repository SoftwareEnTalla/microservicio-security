 ...
 * Licencia: MIT
 *
 * OnSystemAdminPolicyDecisionChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `system-admin-policy-decision` (owner: security). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'system-admin-policy-decision';
...
const TOPICS: string[] = [
  'system-admin-policy-decision-created',
  'system-admin-policy-decision-updated',
  'system-admin-policy-decision-deleted',
];

@Injectable()
export class OnSystemAdminPolicyDecisionChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnSystemAdminPolicyDecisionChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnSystemAdminPolicyDecisionChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnSystemAdminPolicyDecisionChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onSystemAdminPolicyDecisionChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
