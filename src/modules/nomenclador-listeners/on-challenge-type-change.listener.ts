 ...
 * Licencia: MIT
 *
 * OnChallengeTypeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `challenge-type` (owner: security). Suscribe a EventBus local y a Kafka.
 *
...
const TOPICS: string[] = [
  ...
  'challenge-type-updated',
  'challenge-type-deleted',
];

@Injectable()
export class OnChallengeTypeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnChallengeTypeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnChallengeTypeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnChallengeTypeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onChallengeTypeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
