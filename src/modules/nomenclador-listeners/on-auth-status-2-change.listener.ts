 ...
 * Licencia: MIT
 *
 * OnAuthStatus2ChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `auth-status-2` (owner: security). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'auth-status-2';
const OWNER_BC = 'security';
const CROSS_CONTEXT = true;
const EVENT_NAMES: string[] = [
  'AuthStatus2CreatedEvent',
  'AuthStatus2UpdatedEvent',
  'AuthStatus2DeletedEvent',
];
const TOPICS: string[] = [
  'auth-status-2-created',
  'auth-status-2-updated',
  'auth-status-2-deleted',
];

@Injectable()
export class OnAuthStatus2ChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnAuthStatus2ChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnAuthStatus2ChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnAuthStatus2ChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onAuthStatus2Change] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
