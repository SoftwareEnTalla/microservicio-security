 ...
 * Licencia: MIT
 *
 * OnLoginIdentifierTypeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `login-identifier-type` (owner: security). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'login-identifier-type';
...
const TOPICS: string[] = [
  'login-identifier-type-created',
  'login-identifier-type-updated',
  'login-identifier-type-deleted',
];

@Injectable()
export class OnLoginIdentifierTypeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnLoginIdentifierTypeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnLoginIdentifierTypeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnLoginIdentifierTypeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onLoginIdentifierTypeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
