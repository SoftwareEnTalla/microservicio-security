 ...
 * Licencia: MIT
 *
 * OnDeliveryModeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `delivery-mode` (owner: security). Suscribe a EventBus local y a Kafka.
 *
...
const TOPICS: string[] = [
  'delivery-mode-created',
  'delivery-mode-updated',
  'delivery-mode-deleted',
];

@Injectable()
export class OnDeliveryModeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnDeliveryModeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnDeliveryModeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnDeliveryModeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onDeliveryModeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
