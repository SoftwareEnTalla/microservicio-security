 ...
 * Licencia: MIT
 *
 * OnProtocolFamilyChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `protocol-family` (owner: security). Suscribe a EventBus local y a Kafka.
 *
...
const TOPICS: string[] = [
  'protocol-family-created',
  'protocol-family-updated',
  'protocol-family-deleted',
];

@Injectable()
export class OnProtocolFamilyChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnProtocolFamilyChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnProtocolFamilyChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnProtocolFamilyChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onProtocolFamilyChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
