 ...
 * Licencia: MIT
 *
 * OnCertificationStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `certification-status` (owner: security). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'certification-status';
...
const TOPICS: string[] = [
  ...
  'certification-status-deleted',
];

@Injectable()
export class OnCertificationStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnCertificationStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnCertificationStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnCertificationStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onCertificationStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
