 ...
 * Licencia: MIT
 *
 * OnIdentifierTypeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `identifier-type` (owner: security). Suscribe a EventBus local y a Kafka.
 *
...
const TOPICS: string[] = [
  ...
  'identifier-type-updated',
  'identifier-type-deleted',
];

@Injectable()
export class OnIdentifierTypeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnIdentifierTypeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnIdentifierTypeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnIdentifierTypeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onIdentifierTypeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
