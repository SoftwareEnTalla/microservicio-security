...
@Injectable()
export class SystemAdminPolicyDecisionService {
  constructor(private readonly repository: SystemAdminPolicyDecisionRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<SystemAdminPolicyDecision[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<SystemAdminPolicyDecision[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<SystemAdminPolicyDecision[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<SystemAdminPolicyDecision | null> {
  ...
  async update(id: string, partialEntity: Partial<SystemAdminPolicyDecision>): Promise<SystemAdminPolicyDecision | null> {
